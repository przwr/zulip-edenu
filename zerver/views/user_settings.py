...
PRIVACY_SETTINGS_DISABLED_ERROR = gettext_lazy(
    "Privacy settings are disabled in this organization."
)
...
def json_change_settings(
    ...
    *,
    ...
    # PORTAL EDENU: Block privacy settings changes — only the realm owner may
    # change them (the settings UI hides the section from everyone else, so the
    # backend gate matches exactly what is visible)
    if settings.PORTAL_EDENU and not user_profile.is_realm_owner:
        privacy_setting_names = UserBaseSettings.SECURITY_SENSITIVE_USER_SETTINGS
        for setting in set(request_settings.keys()) & privacy_setting_names:
...
def regenerate_api_key(request: HttpRequest, user_profile: UserProfile) -> HttpResponse:
    ...
    if settings.PORTAL_EDENU and not user_profile.is_realm_admin:
